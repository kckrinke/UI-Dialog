...
While UI::Dialog tries to be secure on your behalf, this is not an
easy task and it is advised that you never trust user input when
dealing with any libraries (not just UI::Dialog).

UI::Dialog does not allow for strings with shell command substitutions
by default. This is achived by replacing all instances of back-tick
B<`> characters with single-quotes B<'> and if any B<$()> constructs are
found, the dollar sign is removed.

To disable this behaviour, pass 'trust-input'=>1 into the module
constructor or into any dialog function (such as B<menu>).

See the B<examples/trust-input.pl> sample code for a demonstration.
