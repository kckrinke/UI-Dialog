...
UI::Dialog::GNOME is simply another meta-class for UI::Dialog. This
...
The editbox() widget is also provided in addition to the standard widgets as
both the XDialog and Zenity backends effectively support it's usage. If you
are still using GDialog and not Zenity (which provides a very nice gdialog
wrapper) the editbox() widget will cause your application to die with a
"missing method editbox()" error.
...
=item B<order = [ zenity, xdialog, gdialog ]> (as indicated)
